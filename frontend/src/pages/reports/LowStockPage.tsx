  ...
  return (
    <div className="p-6 max-w-7xl mx-auto min-h-screen">
      <div className="page-header-brand flex flex-col md:flex-row justify-between items-center gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-black text-foreground tracking-tight">Low Stock Alerts</h1>
          <p className="text-slate-400 font-medium">Inventory items currently below their reorder threshold</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div className="bg-card rounded-[32px] p-8 border border-border shadow-sm flex items-center gap-6">
          ...
          <div>
            <p className="text-[10px] uppercase tracking-widest font-black text-muted-foreground">Items to Reorder</p>
        ...
        <div className="bg-card rounded-[32px] p-8 border border-border shadow-sm flex items-center gap-6">
          ...
          <div>
            <p className="text-[10px] uppercase tracking-widest font-black text-muted-foreground">Critical Stockouts</p>
            <h3 className="text-4xl font-black text-foreground">{data.filter(i => i.current_stock === 0).length}</h3>
          </div>
        </div>
      </div>
              ...
              data.map((item) => (
                <TableRow key={item.id} className="hover:bg-white/5 border-border transition-colors group">
                  ...
                  <TableCell className="text-center py-4">
                    <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-black ${item.current_stock === 0 ? 'bg-red-500 text-foreground' : 'bg-red-500/10 text-red-500 border border-red-500/20'}`}>
                      {item.current_stock}
                    </span>
                  </TableCell>
