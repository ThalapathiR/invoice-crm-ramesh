    ...
    <div className="p-6 max-w-7xl mx-auto min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-card rounded-[32px] p-8 border border-border shadow-sm relative overflow-hidden group">
          ...
          <p className="text-[10px] uppercase tracking-widest font-black text-muted-foreground mb-2">Total Items in Stock</p>
          <h3 className="text-4xl font-black text-foreground">{data?.items.reduce((sum: number, i: any) => sum + i.quantity, 0).toLocaleString()}</h3>
        </div>
        
        <div className="bg-card rounded-[32px] p-8 border border-border shadow-sm relative overflow-hidden group">
          <div className="absolute top-0 right-0 p-4 opacity-5 text-emerald-500"><Landmark size={80} /></div>
          <p className="text-[10px] uppercase tracking-widest font-black text-muted-foreground mb-2">Inventory Valuation (CP)</p>
        ...
        <div className="bg-card rounded-[32px] p-8 border border-border shadow-sm relative overflow-hidden group">
          ...
          <p className="text-[10px] uppercase tracking-widest font-black text-muted-foreground mb-2">Low Stock Alerts</p>
          <h3 className={`text-4xl font-black ${lowStockCount > 0 ? 'text-red-500' : 'text-foreground'}`}>{lowStockCount}</h3>
          <p className="text-[10px] font-bold text-muted-foreground mt-1">Items with qty ≤ 5</p>
        </div>
      </div>
              ...
              filteredItems.map((item) => (
                <TableRow key={item.id} className="hover:bg-white/5 border-border transition-colors">
                  ...
                  <TableCell className="py-4">
                    <div className="text-xs font-bold text-muted-foreground">{item.category}</div>
                    <div className="text-[10px] text-slate-500 font-bold">Size: {item.size}</div>
                  </TableCell>
                  <TableCell className="text-center py-4">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-black ${item.quantity <= 5 ? 'bg-red-500/10 text-red-500 border border-red-500/20' : 'bg-muted text-slate-500'}`}>
                      {item.quantity}
                    </span>
                  </TableCell>
                  <TableCell className="text-right font-medium text-slate-500 py-4">₹{item.purchase_price.toLocaleString()}</TableCell>
                  <TableCell className="text-right pr-6 font-black text-foreground py-4">₹{item.stock_value.toLocaleString()}</TableCell>
                </TableRow>
              ))
            )}
          </TableBody>
